...
휴대폰 브라우저로 `https://<내-주소>.vercel.app`을 엽니다 → 본인 선택 → 러닝 앱 스크린샷 한 장 업로드 → **30초 안에 노션 '러닝 기록' 표에 새 줄**이 채워집니다. 🎉
...
- 노션에 줄은 생기는데 칸이 비거나 오류 → **칸 이름이 [02 표](./02-workflow.md#노션-데이터-모델--어떤-표를-미리-만들어-두나)와 정확히 같은지**(괄호·단위까지) 확인. (B) 길에서 가장 흔한 실수입니다.
