...
> 두 갈래 중 **(B)** 입니다.
> - **(A)** 클로드 코드·Codex·Cursor·**Antigravity CLI** 같은 **코딩 에이전트**가 있다 → [`03-tutorial.md`](./03-tutorial.md)로 가세요. 훨씬 빠릅니다.
> - **(B)** 터미널조차 쓰기 싫고, **AI Studio(=브라우저 속 Gemini)** 만으로 → **이 문서**입니다.

> 🟡 **읽기 전에 — 혹시 "유료 도구가 없어서" 이 (B)를 보고 있나요?**
> 구글 **Antigravity CLI**가 **개인 구글 계정으로 무료**입니다(클로드 코드처럼 로컬 파일을 직접 만들어주는 터미널 에이전트, <https://antigravity.google>). 그러면 훨씬 빠른 **(A)** 를 그대로 탈 수 있어요. **(B)** 는 *"터미널을 켜는 것 자체가 싫다"* 는 분께 권합니다. (구 Gemini CLI는 개인용이 2026-06-18 종료 → Antigravity로 이전.)
